...
Begin VB.Form vou1SR 
   Caption         =   "Sale Return Voucher Entry"
   ClientHeight    =   7575
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   11880
   Icon            =   "vou1SR.frx":0000
   LinkTopic       =   "Form1"
   MDIChild        =   -1  'True
   ScaleHeight     =   7575
   ScaleWidth      =   11880
...
Dim p As Long
Set DB = OpenDatabase(Blm.patHmain)
Ssql = "select * from SalesReturn where v_type = 17"
...
Private Function max1() As Double
    ...
    Ssql = "select max(v_no)as c from voumst where v_type = 17"
...
If Option2 = True Then
    ...
    Ssql = "delete from salesReturn where "
    Ssql = Ssql & " v_type = 17"
    Ssql = Ssql & " and v_no = " & Val(Text1.Text)
    DB.Execute Ssql
...
Set TB = DB.OpenRecordset("SalesReturn", dbOpenTable)
For I% = 1 To grid1.Rows - 1
TB.AddNew
            TB.Fields("V_no").Value = Val(Text1.Text)
            TB.Fields("V_Type").Value = 17
...
TB.AddNew
    ...
    TB.Fields("v_no").Value = Val(Text1.Text)
    TB.Fields("v_type").Value = 17
    TB.Fields("narration").Value = "Sale Return JV # " & Text1.Text
TB.Update
TB.Close
...
TB.AddNew
    ...
    TB.Fields("remarks").Value = "(" & grid1.TextMatrix(I%, 2) & " B:" & grid1.TextMatrix(I%, 4) & " W:" & grid1.TextMatrix(I%, 5) & "@" & grid1.TextMatrix(I%, 3) & " ) " & " V#" & Text7.Text
...
If Option1 = True Then
    If date1.Value >= FStartDate And date1.Value <= FEndDate Then
        Text1.Text = max1
    Else
        MsgBox "Please Select a Proper Date in the Financial Year"
    End If
End If
End Sub
...
If Val(Text2.Text) <> 0 Then
    ...
    Else
        ...
        Label10.Caption = Blm.CurrentBalance(Val(Text2.Text))
    End If
