...
Begin VB.Form vou1PR 
   Caption         =   "Purchase Return Voucher Entry"
   ClientHeight    =   7575
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   11880
   Icon            =   "vou1PR.frx":0000
   LinkTopic       =   "Form1"
   MDIChild        =   -1  'True
   ScaleHeight     =   7575
   ScaleWidth      =   11880
...
Dim p As Long
Set DB = OpenDatabase(Blm.patHmain)
Ssql = "select * from PurchaseReturn where v_type = 16"
...
Private Function max1() As Double
    ...
    Ssql = "select max(v_no)as c from voumst where v_type = 16"
...
If Option2 = True Then
    ...
    Ssql = "delete from PurchaseReturn where "
    Ssql = Ssql & " v_type =16"
    Ssql = Ssql & " and v_no = " & Val(Text1.Text)
    DB.Execute Ssql
...
Set TB = DB.OpenRecordset("PurchaseReturn", dbOpenTable)
For I% = 1 To grid1.Rows - 1
TB.AddNew
            TB.Fields("V_no").Value = Val(Text1.Text)
            TB.Fields("V_Type").Value = 16
...
TB.AddNew
    ...
    TB.Fields("v_no").Value = Val(Text1.Text)
    TB.Fields("v_type").Value = 16
    TB.Fields("narration").Value = "Purchase Return V # " & Text1.Text
TB.Update
TB.Close
