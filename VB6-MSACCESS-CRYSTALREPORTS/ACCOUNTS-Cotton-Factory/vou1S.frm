...
Ssql = "Select * from SaleJob where V_no=" & j
'Ssql = Ssql & " and Buyer=" & Val(Text2.Text)
Set RS = DB.OpenRecordset(Ssql)
If Not RS.EOF Then
    Text2.Text = RS.Fields("Buyer").Value
    Text10.Text = Blm.party1(RS.Fields("Buyer").Value)
    Text3.Text = RS.Fields("Item").Value
    Text4.Text = Blm.item1(RS.Fields("Item").Value)
...
If B = False Then
Ssql = "Select Sum(Qty) as Q,Count(bales) as C from sales where JobNo=" & j
    ...
    Select Case RS.Fields("CTerms")
        Case 1 'WT Based
            If RS.Fields("Qty").Value <= RS2.Fields("Q").Value Then
                MsgBox "The Contracted Weight Has Been Sent"
            End If
            lblrecWT.Caption = "Sold Quantity. " & RS2.Fields("Q").Value & ""
            lblBalance.Caption = "Balance : " & RS.Fields("Qty").Value - RS2.Fields("Q").Value
        Case 2
            If RS2.Fields("C").Value >= RS.Fields("Consignments").Value Then
                MsgBox "Contracted Consignments Has Been Sent"
            End If
            lblrecWT.Caption = "Sold Consignments " & RS2.Fields("C").Value & ""
            lblBalance.Caption = "Balance : " & RS.Fields("Consignments").Value - RS2.Fields("C").Value
        Case 3
            If date1.Value < RS.Fields("SDate").Value Or date1.Value > RS.Fields("EDate").Value Then
                MsgBox "Please Check The Voucher Date, As It is Not in the Contracted Period of This Contract"
            End If
    End Select
...
If Not TB.EOF Then
    ...
    Text7.Text = TB.Fields("LorryNo").Value & ""
    Text17.Text = TB.Fields("JoBNo").Value & ""
    Text16.Text = TB.Fields("Days").Value & ""
            grid1.Rows = 1
            Do While Not TB.EOF
                ...
                With grid1
                    ...
                    .TextMatrix(.Rows - 1, 7) = Format(TB.Fields("Amount").Value, "#.00")
                    .TextMatrix(.Rows - 1, 8) = TB.Fields("Remarks").Value
...
With grid1
    ...
    .TextMatrix(.Rows - 1, 6) = Format(Val(Text6.Text), "#.00")
    .TextMatrix(.Rows - 1, 7) = Text12.Text
    .TextMatrix(.Rows - 1, 8) = Text15.Text
...
TB.AddNew
            ...
            TB.Fields("Lorryno").Value = Text7.Text
            TB.Fields("Jobno").Value = Val(Text17.Text)
            TB.Fields("Days").Value = Val(Text16.Text)
    ...
    With grid1
            ...
            TB.Fields("Amount").Value = Val(.TextMatrix(I%, 7))
            TB.Fields("Remarks").Value = .TextMatrix(I%, 8)
...
TB.AddNew
    ...
    TB.Fields("remarks").Value = "(" & grid1.TextMatrix(I%, 2) & " B:" & grid1.TextMatrix(I%, 4) & " W:" & grid1.TextMatrix(I%, 5) & "@" & grid1.TextMatrix(I%, 3) & " ) " & grid1.TextMatrix(I%, 6) & " V#" & Text7.Text
    TB.Fields("debit").Value = Val(grid1.TextMatrix(I%, 7))
...
TB.AddNew
    ...
    TB.Fields("credit").Value = Val(grid1.TextMatrix(I%, 7))
...
If Val(Text17.Text) > 0 Then Cancel = VerifyJob(Val(Text17.Text))
End Sub
...
If Val(Text2.Text) <> 0 Then
    ...
    Else
        ...
        Label14.Caption = Blm.CurrentBalance(Val(Text2.Text))
    End If
...
Dim STAmt As Double
Dim AMT As Double
AMT = Val(Text5.Text) * Val(Text8.Text)
If Val(Text6.Text) > 0 Then STAmt = (AMT * Val(Text6.Text)) / 100
Text12.Text = AMT + STAmt
If grid1.Rows > 1 Then
    For I = 1 To grid1.Rows - 1
        deb = deb + Val(grid1.TextMatrix(I, 4))
        cred = cred + Val(grid1.TextMatrix(I, 5))
        A = A + Val(grid1.TextMatrix(I, 7))
    Next I
    Label8.Caption = deb
    Label9.Caption = Format(cred, "#.000")
    Label11.Caption = A
