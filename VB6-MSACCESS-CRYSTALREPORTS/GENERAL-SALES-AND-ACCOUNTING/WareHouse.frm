...
If Option2 = True Then
    ssql = "delete from WareHouse where code = " & Val(Text1.Text)
    db.Execute ssql
End If


Set tb = db.OpenRecordset("WareHouse", dbOpenTable)
tb.AddNew
...
ssql = "SELECT * FROM WareHouse WHERE NAME = '" & s & "'"
Set db = OpenDatabase(blm.pathMain)
...
ssql = "select MAX(CODE) AS C FROM WareHouse"
Set db = OpenDatabase(blm.pathMain)
Set tb = db.OpenRecordset(ssql)
...
ssql = "SELECT * FROM WareHouse WHERE code = " & Val(Text1.Text)
Set db = OpenDatabase(blm.pathMain)
Set tb = db.OpenRecordset(ssql)
If Not tb.EOF Then
...
If b = True Then
    MsgBox "WareHouse ALREADY EXIST,,,,"
    Text2.SetFocus
End If
End If
End Sub
