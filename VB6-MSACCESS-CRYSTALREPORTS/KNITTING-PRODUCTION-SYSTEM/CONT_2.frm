...
Begin VB.Form cont_S 
   Caption         =   "Sale Contract Entry (Knitting)"
   ClientHeight    =   7185
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   11670
   Icon            =   "CONT_2.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MDIChild        =   -1  'True
   ScaleHeight     =   7185
   ...
   Begin VB.Frame Frame3 
      ...
      Begin VB.ComboBox Combo4 
         Height          =   315
         ItemData        =   "CONT_2.frx":030A
         Left            =   7680
         List            =   "CONT_2.frx":0314
...
Dim ssql As String
ssql = "SELECT * FROM CONT_1 WHERE E_Type = 2 and CONT_NO = " & Val(Text1.Text)
...
If Not tb.EOF Then
    ...
    If Not IsNull(tb.Fields("reference")) Then
...
If Option2 = True Then
    Dim ssql As String
        ssql = "DELETE FROM CONT_1 WHERE E_Type=2 and CONT_NO = " & Val(Text1.Text)
        CN.Execute (ssql)
End If
...
ssql = "select MAX(CONT_NO) AS C FROM CONT_1 where e_type=2"
Set tb = CN.Execute(ssql)
